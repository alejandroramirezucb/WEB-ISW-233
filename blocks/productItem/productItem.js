...
export default class ProductItem extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.querySelector('h3').textContent = product.title || product.name;
    this.querySelector('img').src = `${product.imageUrl}`;
    this.querySelector('img').alt =
      product.altText || product.title || 'Product image';
    this.querySelector('p.product-card__description').textContent =
      `${product.description}`;
    this.querySelector('p.product-card__price').textContent =
      `${product.price}`;
